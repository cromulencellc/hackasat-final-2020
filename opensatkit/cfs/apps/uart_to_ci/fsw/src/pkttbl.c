/*
** Purpose: UART_TO_CI Packet Table.
**
** Notes:
**   None
**
...
boolean PKTTBL_LoadCmd(TBLMGR_Tbl *Tbl, uint8 LoadType, const char* Filename)
{
   ...
   CFE_EVS_SendEvent(UART_TO_CI_INIT_DEBUG_EID, UART_TO_CI_INIT_EVS_TYPE, "PKTTBL_LoadCmd() Entry. sizeof(PktTbl->Tbl) = %d\n", sizeof(PktTbl->Tbl));
   ...
   */
   ...
   if (JSON_OpenFile(JSON_OBJ, Filename)) {
      ...
      CFE_EVS_SendEvent(UART_TO_CI_INIT_DEBUG_EID, UART_TO_CI_INIT_EVS_TYPE, "PKTTBL_LoadCmd() - Successfully prepared file %s\n", Filename);
...
static boolean PktCallback (int TokenIdx)
{
   ...
   CFE_EVS_SendEvent(UART_TO_CI_INIT_DEBUG_EID, UART_TO_CI_INIT_EVS_TYPE,
