...
boolean SCHEDULER_ConfigSchEntryCmd(void* ObjDataPtr, const CFE_SB_MsgPtr_t MsgPtr)
{
   ...
   if (ConfigSchEntryCmd->Slot < SCHTBL_SLOTS) {
      ...
      if (ConfigSchEntryCmd->Activity < SCHTBL_ACTIVITIES_PER_SLOT) {
         ...
         CFE_EVS_SendEvent (SCHEDULER_CFG_SCH_CMD_INFO_EID, CFE_EVS_INFORMATION, 
                     "Processed Sched Entry Cmd Slot %d Activity %d Enabled %d",
                     ConfigSchEntryCmd->Slot, ConfigSchEntryCmd->Activity, Scheduler->SchTbl.Entry[entry].Enabled);
      }
      else {
...
boolean SCHEDULER_LoadSchEntryCmd(void* ObjDataPtr, const CFE_SB_MsgPtr_t MsgPtr)
{
   ...
   if (LoadSchEntryCmd->Slot < SCHTBL_SLOTS) {
      ...
      if (LoadSchEntryCmd->Activity < SCHTBL_ACTIVITIES_PER_SLOT) {
         ...
         CFE_EVS_SendEvent (SCHEDULER_CFG_SCH_CMD_INFO_EID, CFE_EVS_INFORMATION, 
            "SCHEDULER_LoadSchEntryCmd entry: %hu Enabled: %d Frequency: %hu Offset: %hu MsgId: %hu",
            entry, Scheduler->SchTbl.Entry[entry].Enabled, Scheduler->SchTbl.Entry[entry].Frequency, Scheduler->SchTbl.Entry[entry].Offset, Scheduler->SchTbl.Entry[entry].MsgTblEntryId);
   ...
   for (EntryNumber = 0; EntryNumber < SCHTBL_ACTIVITIES_PER_SLOT; EntryNumber++) {
      ...
      if (NextEntry->Enabled == TRUE) {
         ...
         if (Remainder == NextEntry->Offset) {
            ...
            CFE_EVS_SendEvent(SCHEDULER_DEBUG_EID, CFE_EVS_DEBUG,"Scheduler ProcessNextSlot(): slot %d, entry %d, msgid %d", Scheduler->NextSlotNumber, EntryNumber, NextEntry->MsgTblEntryId);
