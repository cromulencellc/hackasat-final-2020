...
static inline void MemCpy(uint8_t *dst, const uint8_t *src, size_t len)
{
   for (; len > 0; len--)
      dst[len-1] = src[len-1];
}  

uint64_t host_to_be_64(uint64_t value) {
#ifdef LE_MACHINE
  uint64_t byte8 = (value >> 56) & 0xff;
  uint64_t byte7 = (value >> 48) & 0xff;
  uint64_t byte6 = (value >> 40) & 0xff;
  uint64_t byte5 = (value >> 32) & 0xff;
  uint64_t byte4 = (value >> 24) & 0xff;
  uint64_t byte3 = (value >> 16) & 0xff;
  uint64_t byte2 = (value >> 8)  & 0xff;
  uint64_t byte1 =  value        & 0xff;
...
void sha512Final(Sha512Context *context, uint8_t *out)
{
   uint32_t i;
   size_t paddingSize;
   uint64_t totalSize;
   ...
   context->w[14] = 0;
   context->w[15] = host_to_be_64(totalSize);
   ...
   for(i = 0; i < 8; i++)
   {
      context->h[i] = host_to_be_64(context->h[i]);
   }
   MemCpy(out, (const uint8_t *)&context->digest, SHA512_DIGEST_SIZE);
}
